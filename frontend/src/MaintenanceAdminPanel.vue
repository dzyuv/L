...
const identifiableAssets = computed(() => {
  const ticket = selectedTicket.value;
  if (!ticket) return [];
  const resourceId = Number(ticket.resourceId) || null;
  const currentId = Number(form.value.assetId) || Number(ticket.assetId) || null;
  return props.assets.filter((item) => {
    if (currentId && Number(item.id) === currentId) return true;
    if (resourceId) return Number(item.resourceId) === resourceId;
    return true;
  });
});
...
<template>
  <div v-if="notice && !selectedTicket" class="ticket-notice" :class="{ failed }">{{ notice }}<button title="关闭提示" @click="notice = ''"><X :size="14" /></button></div>
  ...
  <div v-if="selectedTicket" class="ticket-modal-bg" @click.self="selectedTicket = null"><section class="ticket-modal"><div class="ticket-modal-title"><div><h2>工单处理</h2><p>{{ selectedTicket.ticketNo }}</p></div><button class="modal-close" title="关闭" aria-label="关闭工单窗口" @click="selectedTicket = null"><X :size="22" /></button></div>
    <div v-if="notice" class="ticket-form-notice" :class="{ failed }" role="alert">{{ notice }}<button type="button" title="关闭提示" @click="notice = ''"><X :size="14" /></button></div>
