...
import {
  ...
  FlaskConical, Plus, RefreshCw, Save, Search, Settings, Shield, Users, X,
...
const tabs = computed(() => isSystemAdmin.value
  ? [{ id: "overview", label: "总览", icon: BarChart3 }, { id: "users", label: "用户与角色", icon: Users }, { id: "flows", label: "审批流", icon: ClipboardCheck }, { id: "configs", label: "系统配置", icon: Settings }, { id: "statistics", label: "全局统计", icon: BarChart3 }]
  : [{ id: "overview", label: "运营总览", icon: BarChart3 }, { id: "resources", label: "资源管理", icon: FlaskConical }, { id: "bookings", label: "预约查询", icon: CalendarClock }, { id: "approvals", label: "二级审批", icon: ClipboardCheck }, { id: "violations", label: "违约处理", icon: AlertTriangle }]);
...
function emptyResource() {
  return { typeId: "", name: "", location: "", capacity: 1, description: "", maxDurationMinutes: 120, needCheckin: true };
}
function show(message, failed = false) { notice.value = message; error.value = failed; }
...
async function loadAll() {
  ...
  try {
    if (isSystemAdmin.value) {
      const [userResponse, roleResponse, configResponse, statsResponse, flowResponse, typeResponse] = await Promise.all([
        ...
        axios.get("/api/v1/admin/approval-flows"), axios.get("/api/v1/admin/resource-types"),
      ]);
...
function editResource(item) { resourceForm.value = { typeId: item.typeId, name: item.name, location: item.location, capacity: item.capacity, description: item.description || "", maxDurationMinutes: item.maxDurationMinutes, needCheckin: item.needCheckin }; selectedResource.value = item; resourceDialog.value = true; }
...
async function manageResource(item) {
  ...
  try {
    const [scheduleResponse, closureResponse] = await Promise.all([
      axios.get(`/api/v1/admin/resources/${item.id}/schedules`), axios.get(`/api/v1/admin/resources/${item.id}/closures`),
    ]);
...
<template>
  <main class="admin-shell">
    ...
    <section class="admin-content">
      <div class="admin-topline"><div><span class="eyebrow">ADMINISTRATION</span><h1>{{ tabs.find(item => item.id === activeTab)?.label }}</h1></div><button class="icon-action" :disabled="loading" title="刷新管理数据" @click="loadAll"><RefreshCw :size="18" /></button></div>
      <div v-if="notice" class="admin-notice" :class="{ error }"><AlertTriangle v-if="error" :size="16" /><Check v-else :size="16" />{{ notice }}<button title="关闭提示" @click="notice = ''"><X :size="15" /></button></div>
      ...
      <template v-if="activeTab === 'overview'">
        ...
        <div class="metric-strip" v-else><div><span>管理资源</span><strong>{{ resources.length }}</strong><small>实验室与设备</small></div><div><span>待审批</span><strong>{{ approvals.length }}</strong><small>分配给你的任务</small></div><div><span>待处理违约</span><strong>{{ openViolations }}</strong><small>需要管理员确认</small></div><div><span>待审批预约</span><strong>{{ pendingBookings }}</strong><small>当前占用时间片</small></div></div>
