        ...
        <template v-if="!isSystemAdmin">
          ...
          <section class="report-grid">
            <article class="admin-section report-card"><div class="report-card-head"><div><h2>用户预约次数</h2><small>统计窗口内的有效预约</small></div></div>
              <div class="report-status-list"><div v-for="item in (stats?.userBookings || []).slice(0, 8)" :key="item.userId"><span>{{ statsUserLabel(item.userId) }}</span><strong>{{ item.count }}</strong><em>次</em></div><div v-if="!(stats?.userBookings || []).length" class="admin-empty">暂无用户预约统计</div></div>
            </article>
            <article class="admin-section report-card"><div class="report-card-head"><div><h2>用户违约次数</h2><small>统计窗口内生成的违约记录</small></div></div>
              <div class="report-status-list"><div v-for="item in (stats?.userViolations || []).slice(0, 8)" :key="item.userId"><span>{{ statsUserLabel(item.userId) }}</span><strong>{{ item.count }}</strong><em>次</em></div><div v-if="!(stats?.userViolations || []).length" class="admin-empty">暂无违约统计</div></div>
            </article>
          </section>
