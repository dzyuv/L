...
import {
  ...
  FlaskConical, PackageSearch, Plus, RefreshCw, Save, Search, Settings, Shield, ScrollText, Upload, Users, Wrench, X,
...
const teachers = ref([]);
const roles = ref([]);
const userImportOpen = ref(false);
const configs = ref([]);
...
function configLabel(item) {
  return item?.description || ({
    "checkin.window.before_minutes": "签到提前时间（分钟）",
    "checkin.window.after_minutes": "签到延后时间（分钟）",
    "booking.default_max_duration": "默认最大预约时长（分钟）",
    "booking.slot_minutes": "默认预约粒度（分钟）",
    "violation.max_count": "最大违约次数",
    "violation.restriction_days": "违约限制天数",
    "approval.timeout_minutes": "审批超时时间（分钟）",
  })[item?.key] || item?.key || "配置项";
}
function configTypeText(type) {
  return ({ INT: "整数", STRING: "文本", BOOLEAN: "开关" })[type] || type || "文本";
}
function operationName(code) { return ({ USER_STATUS_UPDATED: "修改用户状态", USER_ROLES_UPDATED: "调整用户角色", USER_PASSWORD_RESET: "重置用户密码", USER_DELETED: "删除用户", USER_IMPORTED: "导入用户", APPROVAL_FLOW_PUBLISHED: "发布审批流", SYSTEM_CONFIG_UPDATED: "修改系统配置" })[code] || code; }
...
function operationDetail(item) {
  ...
  if (item.operationType === "USER_IMPORTED") return `成功 ${detail.createdCount || 0} 人，跳过 ${detail.skippedCount || 0} 人，失败 ${detail.failedCount || 0} 人`;
  if (item.operationType === "APPROVAL_FLOW_PUBLISHED") return `版本 v${detail.version || "-"}，${detail.nodeCount || 0} 个审批节点`;
  if (item.operationType === "SYSTEM_CONFIG_UPDATED") return configLabel({ key: detail.key });
  return item.reason || "-";
}
...
async function saveConfig(item) {
  try { await axios.put(`/api/v1/admin/configs/${encodeURIComponent(item.key)}`, { value: item.value }); show(`${configLabel(item)} 已保存`); await loadAll(); }
...
async function cancelClosure(item) {
  try {
    const response = await axios.post(`/api/v1/admin/resources/${selectedResource.value.id}/closures/${item.id}/cancel`);
    const restored = Number(response.data?.data?.restoredCount || 0);
    show(restored > 0 ? `维护时段已取消，已恢复 ${restored} 条尚未开始的预约` : "维护时段已取消");
    await manageResource(selectedResource.value);
  }
...
<template>
  <main class="admin-shell">
    ...
    <UserImportModal v-if="userImportOpen" @close="userImportOpen = false" @imported="loadAll" />
...
.user-toolbar{gap:12px}.toolbar-actions{display:flex;align-items:center;gap:8px}
@media(max-width:760px){.user-toolbar{height:auto;align-items:stretch;flex-direction:column}.user-toolbar .search{width:100%}.toolbar-actions{justify-content:space-between}}
</style>
